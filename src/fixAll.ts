...
export async function getTsLintFixAllCodeAction(document: TextDocument, diagnostics: Diagnostic[]): Promise<CodeAction | undefined> {
  let end: Position = document.positionAt(document.getText().length)
  let range = Range.create(0, 0, end.line, end.character)
  const map = await languages.getCodeActions(document, range, { diagnostics })
